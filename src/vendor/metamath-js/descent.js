...
// Vendored from google/metamath.js src/descent.js — see NOTICE.md.
// Only change: CommonJS require/module.exports converted to ES import/export.

import { lexicon } from "./lexer.js";
import { MM } from "./engine.js";
import moo from "moo";
...
export class Parser {
  ...
  error() {
    ...
    throw new Error(
      `Syntax error on line ${line} column ${col}: unexpected ${this.head.value.type} token.`,
    );
  }
  block() {
    this.eat("lscope");
...
export function parse(code, handler) {
  const parser = new Parser(handler);
  const stream = tokens(code);
  return parser.parse(stream);
}

export function process(program) {
  const mm = new MM();
  mm.push();
...
export class Verifier {
  verify(program, label) {
    ...
    for (const [, [, , proof]] of Object.entries(mm.labels).filter(
      ([, [type]]) => type == "$p",
    )) {
      proof();
      proofs++;
