...
// Vendored from google/metamath.js src/metamath.js — see NOTICE.md.
...
export class Stack {
  ...
  addD([d, vars]) {
    ...
    if (vars.length < 2) {
      throw new Error(
        `Invalid disjoint statement: need at least two variables.`,
      );
    }
    ...
    for (const variable of vars) {
      if (!declared(variable)) {
        throw new Error(
          `Disjoint statement of undeclared variable ${variable}.`,
        );
      }
    }
    ...
    for (const [hyp] of [...e, [rule, type]]) {
      for (const tok of hyp) {
        if (this.lookupV(tok) && this.lookupF(tok)) {
          ...
          throw new Error(
            `Undeclared token: "${tok}" is neither a constant nor a variable.`,
          );
        }
      }
    }
    ...
    for (const stmt of statements) {
      ...
      if (first == "$c") {
        ...
        if (proof[0] != "(") {
          ...
          const labels = [];
          const args = f.map(([k, v]) => v).map((v) => this.frames.lookupF(v));
          const hyps = e.map(([rule, type]) => this.frames.lookupE(rule, type));
          labels.push(...args);
          labels.push(...hyps);
          ...
          proof.explode = () => {
            const [, external, , compressed] = proof;
            return new Decompressor().explode(
              labels,
              external,
              compressed,
              that.labels,
  ...
  verify(label, type, theorem, proof, generate) {
    ...
    for (const step of proof) {
      if (step == -1) {
        ...
        if (!markers[step]) {
          throw new Error(
            `Can't find marker for ${step}: ${Object.keys(markers)}.`,
          );
        }
        ...
        if (!this.labels[step]) {
          throw new Error(
            `Unknown theorem "${step}" in the proof for "${label}".`,
          );
        }
        ...
        if (op == "$e" || op == "$f") {
          ...
          if (sp < 0) {
            throw new Error(
              `Step ${step}: empty stack ${sp} (length = ${stack.length} and popping = ${npop}).`,
            );
          }
          ...
          for (const [k, v] of mandatory) {
            ...
            if (top[1] != k) {
              console.log(
                `Stack at ${sp} because ${mandatory.length} args + ${hyp.length} hypothesis:`,
              );
              for (let [index, type, string] of stack.reverse()) {
                console.log(`  ${type} ${string.flat().join(" ")}`);
              }
              throw new Error(
                `Step "${step}" of "${label}": argument type of "${v}" doesn't match with the top of the stack. Expected "${k}" but got "${top[1]}" (${top[2].flat().join(" ")}).`,
              );
            }
            subs[v] = top[2];
            args.push(top[0]);
          ...
          for (const [h, type] of hyp) {
            ...
            if (top[1] != type) {
              throw new Error(
                `Step ${step}: argument type doesn't match with the top of the stack. Expected ${type} but got ${top[1]}.`,
              );
            }

            const sub = h.map((tok) => (subs[tok] ? subs[tok] : tok));
            if (top[2].flat().join("") != sub.flat().join("")) {
              const e = [];
              e.push(
                `Substitution ${JSON.stringify(subs)} of the hypothesis ${h.join(" ")} doesn't match with the top of the stack`,
              );
              e.push(
                `Step ${step}: Expected ${sub.flat().join(" ")} but got ${top[2].flat().join(" ")}.`,
              );
            ...
            for (const [x, y] of dvs) {
              ...
              for (let el1 of a) {
                for (let el2 of b) {
                  if (el1 == el2) {
                    throw new Error(
                      `${x} (=${a}) and ${y} (=${b}) are disjoined variables and can't carry the same value. `,
                    );
                  }
                  if (!this.frames.lookupD(el1, el2)) {
                    throw new Error(
                      `${el1} of ${x} (${a}) and ${el2} of ${y} (${b}) aren't declared as disjoint.`,
                    );
                  }
                }
              }
          ...
          const el = result[1].map((tok) => (subs[tok] ? subs[tok] : tok));
    ...
    if (type != kind) {
      throw new Error(
        `Assertion proved doesn't match: expected ${type} but got ${kind}`,
      );
    }

    if (last.flat().join(" ") != theorem.flat().join(" ")) {
      throw new Error(
        `Result of proof doesn't match assertion: expected "${theorem.join(" ")}" but got "${last.join(" ")}".`,
      );
    }
